...
class PointNetv2Seg(nn.Module):
    def __init__(
        self, num_class=1, in_channel=4, with_decoder=False, out_dim=128,
...
if __name__ == "__main__":
    ...
    batch_size = 32 * 4
    num_points = 1500
    input_xyz = torch.rand(batch_size, num_points, 4).cuda()  # 输入形状为 [batch_size, num_points, 3]

    # 初始化模型
    model = PointNetv2Seg().cuda()
